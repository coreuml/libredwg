                  ...
                  Dxf_Pair *restrict pair)
{
  ...
  _o->dashes
      = (Dwg_LTYPE_dash *)xcalloc (_o->num_dashes, sizeof (Dwg_LTYPE_dash));
  if (!_o->dashes)
    {
      _o->num_dashes = 0;
      return NULL;
  ...
  for (int j = -1; j < num_dashes;)
    {
      if (!pair || pair->code == 0)
      ...
      else if (pair->code == 9)
        {
          ...
          if (!_o->strings_area)
            _o->strings_area = (BITCODE_TF)xcalloc (is_tu ? 512 : 256, 1);
          if (is_tu)
            {
                      ...
                      Dxf_Pair *restrict pair)
{
  ...
  _o->lines = (Dwg_MLINESTYLE_line *)xcalloc (num_lines,
                ...
                Dxf_Pair *restrict pair)
{
  ...
  _o->points = (BITCODE_2RD*)xcalloc (num_points, sizeof (BITCODE_2RD));
  if (!_o->points)
    {
      _o->num_points = 0;
      return NULL;
  ...
  while (pair != NULL && pair->code != 0)
    {
      ...
      if (pair == NULL || pair->code == 0)
      ...
      else if (pair->code == 42)
        {
          if (!_o->num_bulges)
            {
              _o->bulges = (BITCODE_BD*)xcalloc (num_points, sizeof (BITCODE_BD));
              if (!_o->bulges)
                {
                  _o->num_bulges = 0;
                  return NULL;
      ...
      else if (pair->code == 91)
        {
          if (!j)
            {
              _o->vertexids = (BITCODE_BL*)xcalloc (num_points, sizeof (BITCODE_BL));
              if (!_o->vertexids)
                {
                  _o->num_vertexids = 0;
                  return NULL;
      ...
      else if (pair->code == 40) // not const_width
        {
          if (!j)
            {
              _o->widths = (Dwg_LWPOLYLINE_width *)xcalloc (
  ...
  while (pair != NULL && pair->code == 1)
    {
      ...
      if (!total)
        ...
        {
          total += len;
          o->encr_sat_data[0] = (char*)realloc (o->encr_sat_data[0], total + 1);
          if (!o->encr_sat_data[0])
            {
              LOG_ERROR ("Out of memory");
              return NULL;
  ...
  while (pair->code != 0)
    {
      ...
      if (pair == NULL || pair->code == 0)
      ...
      else if (pair->code == 93)
        {
          ...
          if (pair->value.u)
            {
              o->faces = (BITCODE_BL *)xcalloc (o->num_faces, sizeof (BITCODE_BL));
              if (!o->faces)
                {
                  o->num_faces = 0;
                  return NULL;
      ...
      else if (pair->code == 95)
        {
          ...
          if (pair->value.u)
            {
              o->crease = (BITCODE_BD*)xcalloc (o->num_crease, sizeof (BITCODE_BD));
              if (!o->crease)
                {
                  o->num_crease = 0;
                  return NULL;
      ...
      else if (pair->code == 90)
        {
          if (vector == 93)
          ...
          else if (vector == 95)
            {
              ...
              if (pair->value.u) // from face - to face
                {
                  o->crease = (double*)xcalloc (o->num_crease, sizeof (double));
                  if (!o->crease)
                    {
                      o->num_crease = 0;
                      return NULL;
      ...
      if (pair == NULL || pair->code == 0 || pair->code == 75)
      ...
      else if (pair->code == 95 && !is_plpath)
        {
          ...
          LOG_TRACE ("HATCH.paths[%d].segs[%d].num_knots = %ld [BL 95]\n", j,
                     k, pair->value.l);
          o->paths[j].segs[k].knots = (double*)xcalloc (pair->value.l, sizeof (double));
      ...
      while (pair != NULL && pair->code != 301 && pair->code != 0)
        {
          switch (pair->code)
            ...
            case 47:
              ...
              if (!j)
                {
                  ctx->content.blk.transform = (double *)xcalloc (16, sizeof (double));
  ...
  while (pair != NULL)
    {
      switch (pair->code)
        ...
        case 93:
          ...
          o->geomesh_pts = (Dwg_GEODATA_meshpt*)xcalloc (pair->value.u, sizeof (Dwg_GEODATA_meshpt));
        ...
        case 96:
          ...
          o->geomesh_faces = (Dwg_GEODATA_meshface*)xcalloc (pair->value.u, sizeof (Dwg_GEODATA_meshface));
          ...
          if (mode == TABLEFORMAT)
            {
              ...
              LOG_TRACE ("%s.%s.num_borders = " FORMAT_BL " [BL %d]\n",
                         obj->name, key, pair->value.u, pair->code);
              o->borders = (Dwg_GridFormat*)xcalloc (o->num_borders, sizeof (Dwg_GridFormat));
            }
          else if (mode == CONTENTFORMAT)
            {
                        ...
                        Dxf_Pair *restrict pair)
{
  ...
  o->cells = (Dwg_TABLEGEOMETRY_Cell*)xcalloc (num_cells, sizeof (Dwg_TABLEGEOMETRY_Cell));
  if (!o->cells)
    {
      o->num_cells = 0;
      return NULL;
          ...
          if (strEQc (ctrlname, "DIMSTYLE_CONTROL"))
            {
              if (pair->value.u)
                {
                  BITCODE_H *hdls;
                  hdls = (BITCODE_H *)xcalloc (pair->value.u, sizeof (BITCODE_H));
                  if (!hdls)
                    pair->value.u = 0;
            ...
            BITCODE_RS *restrict flagp)
{
  ...
  if (pair->code == 210 || pair->code == 220 || pair->code == 230)
  ...
  else if (pair->code == 72)
    {
      ...
      *jp = 0;
      _o->knots = (BITCODE_BD *)xcalloc (_o->num_knots, sizeof (BITCODE_BD));
      if (!_o->knots)
        {
          _o->num_knots = 0;
          return 0;
  ...
  else if (pair->code == 73)
    {
      ...
      *jp = 0;
      _o->ctrl_pts = (Dwg_SPLINE_control_point *)xcalloc (
          _o->num_ctrl_pts, sizeof (Dwg_SPLINE_control_point));
      if (!_o->ctrl_pts)
        {
          _o->num_ctrl_pts = 0;
          return 0;
  ...
  else if (pair->code == 74)
    {
      ...
      *jp = 0;
      _o->fit_pts = (BITCODE_3BD *)xcalloc (_o->num_fit_pts, sizeof (BITCODE_3BD));
      if (!_o->fit_pts)
        {
          _o->num_fit_pts = 0;
          return 0;
  ...
  if (pair->code == 72)
    {
      ...
      _o->verts = (Dwg_MLINE_vertex *)xcalloc (_o->num_verts, sizeof (Dwg_MLINE_vertex));
      if (!_o->verts)
        {
          _o->num_verts = 0;
          return 0;
      ...
      if (_o->num_lines)
        {
          for (int _j = 0; _j < _o->num_verts; _j++)
            {
              _o->verts[_j].lines
                = (Dwg_MLINE_line*)xcalloc (_o->num_lines, sizeof (Dwg_MLINE_line));
...
postprocess_TEXTlike (Dwg_Object *obj)
{
  ...
  dwg_dynapi_entity_value (_obj, obj->name, "oblique_angle", &oblique_angle, NULL);
        ...
        object_default:
          if (pair->code >= 1000 && pair->code < 1999)
          ...
          else if (pair->code == 90 && obj->fixedtype == DWG_TYPE_OLE2FRAME)
            {
              ...
              _o->data = (BITCODE_RC *)xcalloc (pair->value.l, 1);
              if (!_o->data)
                {
                  _o->data_size = 0;
                  goto invalid_dxf;
          ...
          */
          ...
          else if (obj->fixedtype == DWG_TYPE_MTEXT && pair->code == 50)
            {
              ...
              if (!j)
                {
                  o->column_heights
                    = (BITCODE_BD*)xcalloc (o->num_column_heights, sizeof (BITCODE_BD));
                  if (!o->column_heights)
                    {
              ...
              if (obj->fixedtype == DWG_TYPE_CELLSTYLEMAP)
                {
                  ...
                  if (cur_cell == 0 && !_o->cells)
                    _o->cells = (Dwg_TABLESTYLE_CellStyle *)xcalloc (
                        _o->num_cells, sizeof (Dwg_TABLESTYLE_CellStyle));
                       ...
                       || pair->code == 30))
            {
              ...
              if (!j && pair->code == 10)
                {
                  o->points = (BITCODE_3BD*)xcalloc (o->num_points, sizeof (BITCODE_3BD));
                  if (!o->points)
                    {
                      o->num_points = 0;
                      goto invalid_dxf;
              ...
              for (f = &fields[0]; f->name; f++)
                {
                          ...
                          if (pair->code < 20)
                            {
                              ...
                              if (!j && pair->code == 14)
                                {
                                  clip_verts = (BITCODE_2RD*)xcalloc (num_clip_verts,
                                                       sizeof (BITCODE_2RD));
                                  dwg_dynapi_entity_set_value (_obj, obj->name,
                                                               f->name,
                                                               &clip_verts, 0);
                  ...
                  // FIELD_VECTOR_N BITCODE_BD transmatrix[16]:
                               ...
                               || (pair->code <= 142 && pair->code <= 147)))
                    {
                      ...
                      if (!matrix)
                        {
                          matrix = (BITCODE_BD*)xcalloc (16, sizeof (BITCODE_BD));
                          if (!matrix)
                            goto invalid_dxf;
                          j = 0;
                        }
                          ...
                          Dwg_Object_Ref *restrict ownerhandle)
{
 ...
 if (obj->type == DWG_TYPE_BLOCK)
  ...
  if (!_ctrl->first_entity)
   _ctrl->last_entity =  _ctrl->first_entity = dwg_add_handleref (dwg, 4, obj->handle.value, NULL);
  else
